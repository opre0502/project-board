 ...
 *
 * - Thymesleaf -
 *      뷰 파일은 HTML 로 작업할건데, 타임리프를 설치 함으로서 스프링은 이제 html 파일을 마크업으로 보지않고, 타임리프 템플릿 파일로 인식한다.
...
public class Ex08_4_ArticleController {
    ...
    public String articles(ModelMap map) {
        /* ModelMap: 테스트파일에서 .andExpect(model().attributeExists("articles")); 를 이용해서 articles 라는 키 값으로 데이터를 넣어주기로 했으니까 필요함.
        *
        * Model 과 ModelMap는 같은거임. 차이점은 Model 은 인터페이스, Model은 클래스 구현
        * */
