 ...
 */
/** Aricle과 ArticleComment 에 있는 공통 필드(메타데이터. ID제외) 들을 별도로 빼서 관리 할거임.
 * 이유는 앞으로 Aricle과 ArticleComment 처럼 fk 같은거로 엮여있는 테이블들 만들경우, 모든 domain 안에 있는 파일들에 많은 중복코드
 * 들이 들어가게 된다. 그래서 별도에 파일에 공통되는 것들을 다 몰아 넣고 사용하는거 해보기
 *
 * 참고: 공통필드를 빼는걸 팀마다 다르다. 중복코드를 싫어해서 그냥 각 파일마다 다 두는 사람들이 있고,
 *      (유지보수)
 *
 *      중복코드를 괜찮아 헤서 각 파일에 그냥 두는 사람도 있다.
 *      (각 파일에 모든 정보가 다 있다. 변경시 유연하게 코드 작업을 할 수 있다)
 *
 * 추출은 두가지 방법으로 할 수 있다.
 *      1) @Embedded - 공통되는 필드들을 하나의 클래스로 만들어서 @Embedded 있는 곳에서 치환 하는 방식
 *      2) @MappedSuperClasss - (요즘 실무에서 사용)
 *              @MappedSuperClass 어노테이션이 붙은곳ㅇ서 사용
 *
 *  *둘의 차이: 사실은 둘이 비슷하지만 @Embedded 방식을 하게되면 필드가 하나 추가된다.
 *              영속성 컨텍스트를 통해서 데이터를 넘겨 받아서 어플리케이션으로 열었을때에는 어차피 AuditingField 랑 똑같이 보인다.
 *              (중간에 한단계가 더 있다는 뜻)
 *
 *              @MappedSuperClasss 는 표준 JPA에서 제공해주는 클래스. 중간단계 따로 없이 바로 동작
 * */
