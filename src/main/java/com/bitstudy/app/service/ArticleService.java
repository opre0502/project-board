...
//        return Page.empty();
        ...
        */
        ...
        /** ArtuckeServuceTest 의 withoutKeywordReturnArticlesAll() 부분 관련 실제 구현
         * When 부분에서 searchArticles 메서드를 이용해서 값들을 전달한다.
         * 사실 검색 관련된 부분이기 떄문에 검색어인 searchKeyword를 이용해서 코드를 짤거임.
         *
         * */
        ...
        return switch (searchType){
             ...
             *      코드
             *      break;
             * 이런 식이였는데 이제 '->' 키워드를 이용해서 알아서 리턴까지 할수 있게 발전됨.
             * */
    ...
    public ArticleWithCommentsDto getArticle(Long articleId){
        return articleRepository.findById(articleId)  // articledId 로 게시글 하나 잡을거니까 findById
    ...
    public void saveArticle(ArticleDto dto) {
        /** select 구문때 처럼 DB에서 가져올 정보가 없기 때문에 그냥 void 라서 return 이 없다. */
        articleRepository.save(dto.toEntity());
        // toEntity 를 이용해서 매개변수로 받은 dto(단순히 데이터를 가지고 있기만 한. 누가 어떤 정보인지 모르는 상태) 정보로 부터 엔티티를 하나 만들어서 세이브 하는 코드
    }
    ...
    public void updateArticle(ArticleDto dto){
        try {
            ...
            /** 매개변수 dto 로 받은 title, content 는 not null 필드이기 떄문에 if로 감싸서 null 이 아닌 경우에만 돌도록함. */
            if(dto.title() != null) {
            ...
            article.setHashtag(dto.hashtag()); /** null 가능 필드라서 if문으로 검사할 필요없다. */

        } catch (EntityNotFoundException e) {
            // 수정 정보를 넣었을때 게시글이 없어진 경우(ex 정보수정중 관리자가 게시글을 삭제했다라던가..)
