...
/** 로그인 페이지 기능 테스트
 * 로그인 페이지는 스프링 시큐리티와 부트가 만들어서 준거다. 한마디로 이미 테스트가 다 검증이 되서 우리가 별도로 테스트할건 없다.
 * 그래서 최소한의 테스트만 할것이다. (저 기능이 우리 서비스에 존재하는지만 확인하면 됨.)
 * 그래서 별도로 실제 Authcontroller 파일이 존재하지 않아도 된다.
 *
 * get("/login") 로 페이지를 날렸을때 상태코드가 200 나오면 테스트 통과
 * */
