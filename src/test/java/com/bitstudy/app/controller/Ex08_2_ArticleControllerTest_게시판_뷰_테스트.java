    ...
    *  */
    ...
    public void articlesAll() throws Exception {
        mvc.perform(get("/articles"))
                ...
                // 뷰 를 만들고 있으니까 html로 코드를 짜고있을거임. /articles 로 받아온 데이터의 미디어 타입이 html 타입으로 되어 있는지 확인
                // contentType 의 경우 exact match 라서 미디어타입이 딱 text/html 로 나오는것만 허용하기 때문에
                ...
                // 이 뷰에서는 게시글들이 떠야 하는데, 그 말은 서버에서 데이터들을 가져왔다는 말이다. 그러면 모델 어트리뷰트로 데이터를 밀어넣어줬다는 말인데 그게 있는지 없는지 확인
                // model().attributeExists("articles") <- articles는 개발자가 임의로 걸어주는 키값, 맵에 articles 라는 키가 있는지 검삭해라 라는 뜻.
    }
