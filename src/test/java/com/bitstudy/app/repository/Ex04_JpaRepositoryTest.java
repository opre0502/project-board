...
/** 슬라이드 테스트란 지난번 TDD 떄 메서드들 다 남남으로 서로 알아보지 못하게 만들었었다. 이것처럼 메서드들 각각 테스트한
 * 결과를 서로 못보게 잘라서 만드는것
 */
@Import(JpaConfig.class)
/** 원래대로라면 JPA 에서 모든 정보를 컨트롤 해야되는데 JpaConfig의 경우는 읽어오지 못한다.
(이유: 이건 시스템에서 만든게 아니라 우리가 별도로 만든 파일이기 떄문. 그래서 따로 import를 해줘야 한다.)
...
class Ex04_JpaRepositoryTest {
    ...
    * */
    ...
    /*  - 트랜잭션시 사용하는 메서드
        ...
        1) findAll() - 모든 컬럼을 조회할때 사용. 페이징(pageable) 가능
                        당연히 select 작업을 하지만, 잠깐 사이에 해당 테이블에 어떤변화가 있었는지 알 수 없기 떄문에 select 전에
    ...
    * */
    ...
    void selectTest() {
         ...
         *
         * maven방식: dao -> mapper 로 정보 보내고 DB 갔다 와서 C 까지 돌려보낼건데 dao 에서 DTO를 list에 담아서 return
         *
         * */
    ...
    @Test
    void insertTest() {
        /** 기존의 article 개수 센 다음에, insert 하고, 기존거보다 현재꺼가 1 차이가 나면 insert 제대로 됐다는 뜻 */
        // 기존 카운트 구하기
        Long prevCount = articleRepository.count();

        // insert 하기
        Article article = Article.of("테스트제목","테스트본문","#테스트해쉬태그");
        articleRepository.save(article);

        // 기존꺼랑 현재꺼랑 개수 차이 구하기
        assertThat(articleRepository.count()).isEqualTo(prevCount + 1);

        /* !!주의: 이 성태로 테스트를 돌리면 createAt 이거 못찾는다고 에러남.
    ...
    void updateTest(){
        ...
        /*  순서1) 기존의 영속성 컨텍스트로부터 하나 엔티티 객체를 가져온다.(DB에서 한줄 뽑아온다)
        ...
        */
        ...
        /* 순서2) 업데이트로 해시태그를 바꾸기
        *         엔티티에 있는 setter 를 이용해서 updateHashtag에 있는 문자열로 업데이트 하기
        *   1. 변수 updateHashtag에 바꿀 문자열 저장
        *   2. 엔티티(article)에 있는 setter를 이용해서 변수 updateHashtag에 있는 문자열을 넣고
        *       (해시태그 바꿀꺼니까 setHashtag. 이름 어찌할지 모르겠으면 실제 엔티티 파일
        *       가서 setter 만들어 보기, 그 이름 그대로 쓰면된다.
        *   3. 데이터 베이스에 업데이트 하기          */
        ...
        /** save 로 놓고 테스트 돌리면 콘솔(Run)탭에 update 구문이 나오지 않고 select구문만 나온다.
         * 이유는 영속성 컨텍스트로부터 가져온 데이터를 그냥 save만 하고 아무것도 하지 않고 끝내버리면 어차피
         * 롤백 되나까 스프링부트는 다시 원래의 값으로 돌아가질거다. 그래서 그냥 했다 치고 update를 하지 않는다.
         * (코드의 유효성은 확인) 그래서 save를 하고 flush를 해줘야한다.
         *
         * -flush 란 (push 같은것)
         *  1. 변경점 감지
        ...
        /* 순서3) 위에서 바꾼 savedArticle 에 업데이트 된 hashtag 필드에 updateHashtag에
        *            저장되어 있는 값("#abcd") 이 있는지 확인하자.     */
...
//        assertThat(savedArticle.getHashtag()).isEqualTo(updateHashtag);
    ...
    void deleteTest(){
        ...
        *   - 순서
        *       articleReository -> 기존의 영속성 컨텍스트로부터
        ...
        *     게시글(articleRepository) 뿐만 아니라, 연관된 댓글(articleCommentRepository)
        *     까지 삭제할거라서 두개의 개수 다 알아내기.   */
        ...
        int deletedCommentSize = article.getArticleComments().size(); // 해당 게시글에 딸려 있는 댓글의 수
        // 나중에 "모든 댓글의 수 - 게시글에 딸려있는 댓글 수": 하면 몇개 지워졌는지 알 수 있다.

        /* 3) delete 하고 (전체 게시글 개수 -1됨)*/
        articleRepository.delete(article);

        /* 테스트 동과 조건 - 2번에서 구한거랑 여기서 구하는 개수가 1차이 나는경우 */
